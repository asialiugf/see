...
int main()
{
    ...
    //ret = zmq_bind(publisher, "tcp://127.0.0.1:8887");/// 绑定该发布到TCP通信
    //ret = zmq_bind(publisher, "tcp://localhost:9021");/// 绑定该发布到TCP通信
    ret = zmq_connect(publisher, "tcp://localhost:9021");
    assert(ret == 0);
