...
int
errlog( int iErrCode, char *pcaDispMsg, char cDumpDev, char *pcaDumpAddr, long llen )
{
  int iRc = 0 ;
  ...
  if(cDumpDev & RPT_TO_LOG ){
      ...
      if (iErrCode == NO_ERR) {
          ...
          disp_errmsg(iUcpErLog,pcaDispMsg,pcaDumpAddr,llen); 
      }
  ...
  if (cDumpDev & RPT_TO_TTY ) {    /* output to tty */
    iTtyDev = 1;
      if (iErrCode == NO_ERR) {
        ...
        if ((LOG_MODE == 1) || (iErrCode > 999))
          disp_errmsg(iTtyDev,pcaDispMsg,pcaDumpAddr,llen);
      }
  }
  ...
  if (cDumpDev & RPT_TO_CON ) {    /* output to console */
    if ( (iConsole=open(CONSOLE,O_WRONLY)) == -1 ) {
      ...
      if (iErrCode == NO_ERR) {
        ...
        if ((LOG_MODE == 1) || (iErrCode > 999))
          disp_errmsg(iConsole,pcaDispMsg,pcaDumpAddr,llen);
      }
      close(iConsole);
    }
  }
...
void
disp_errmsg(int iFileId,char *pcaMsgStr,char *pcaDumpAddr,long llen)
{
  long lTLoc;
  long lCurTime;
  char caStrBuf[300];
  ...
  if((pcaDumpAddr != 0)&&(llen > 0)){
    // mem_dump(iFileId,pcaDumpAddr,llen);
     DumpHex(iFileId,(unsigned char *)pcaDumpAddr,llen);
  }
  write(iFileId,"\n",1);
}
...
void
mem_dump(int iFileId,char *pucaAddr,long llen)
{
