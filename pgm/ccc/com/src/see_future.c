...
int see_block_init( see_fut_block_t * p_block, char * pc_future, see_hours_t t_hours[] )
{
    ...
    bar_block.bar0.o = SEE_NULL; bar_block.bar1.o = SEE_NULL;
    bar_block.bar0.c = SEE_NULL; bar_block.bar1.c = SEE_NULL;
    bar_block.bar0.h = SEE_NULL; bar_block.bar1.h = SEE_NULL;
    bar_block.bar0.l = SEE_NULL; bar_block.bar1.l = SEE_NULL;
    bar_block.bar0.v = 0 ; bar_block.bar1.v = 0 ;
    bar_block.c_save = 0 ;
    ...
    for ( i=0;i<8;i++ ) {       p_block->bar_block[i].c_bar_type  = BAR_SECOND ; }
    for ( i=8;i<16;i++ ) {      p_block->bar_block[i].c_bar_type  = BAR_MINUTE ; }
    for ( i=16;i<25;i++ ) {     p_block->bar_block[i].c_bar_type  = BAR_HOUR ; }
    ...
    while ( fgets(ca_line,1024, pf_future_time ) != NULL ){
        ...
        if ( ca_future[0] == ca_line[0]  ) {
            ...
            if ( ca_future[1] == ca_line[1] ) {
                i = 2 ;
                while ( ca_line[i] > '9' || ca_line[i] < '0' ) { i++; }
...
int see_trading_hours_init( see_hours_t t_hours[] )
{
    ...
    while ( fgets(ca_line,1024, pf) != NULL ){
        ...
        for ( i=1 ; i<k; i++ ){
            ...
            if( (i-1)%2  ==0 ) { memcpy(ca_30F_s,ca_segment[i],8); }
            if( (i-1)%4  ==0 ) { memcpy(ca_1H_s, ca_segment[i],8); }
            if( (i-1)%8  ==0 ) { memcpy(ca_2H_s, ca_segment[i],8); }
            if( (i-1)%12 ==0 ) { memcpy(ca_3H_s, ca_segment[i],8); }
            if( (i-1)%16 ==0 ) { memcpy(ca_4H_s, ca_segment[i],8); }
            if( (i-1)%20 ==0 ) { memcpy(ca_5H_s, ca_segment[i],8); }
            if( i-1 ==0 ) { memcpy(ca_1D_s,ca_segment[i],8); }
