...
#property copyright "Copyright 2011 by Capella at http://www.worldwide-invest.org"
...
extern string UseIndicatorSet = "=== Indicators: 1 = Moving Average, 2 = BollingerBand, 3 = Envelopes, 4 = ATR ==";
extern int UseIndicatorSwitch = 1;  	  		// Switch User indicators. Note for 4 (iATR), UseVolatilityPercentage is forced to FALSE
extern double BBDeviation = 1.50; 				// Deviation for the iBands indicator
extern double EnvelopesDeviation = 0.01;     // Deviation for the iEnvelopes indicator
extern double VolatilityATRMultiplier = 3;	// Multiplication factor for the ATR-indicator	
extern bool ATRDualTrade = FALSE;				// If set to TRUE then ATR	will open up trades in both directions at the same time												
...
string ea_version = "XMT-Scalper v. 2.3.4";
...
void sub_trade() 
{
   ...
   double local_envelopesdiff;	
	double local_atrfast;
   double local_atrslow;	
	double local_atrmultiplied;
	double local_atrvolatility;
   ...
   double local_skipticks;
	...
	// Calculate the channel of Volatility based on ATR during current bar
	if (UseIndicatorSwitch == 4)
	{
		local_atrfast = iATR(Symbol(),PERIOD_M1, 1, 0);
		local_atrslow = iATR(Symbol(),PERIOD_M1, 60, 0);
		local_atrmultiplied = local_atrslow * VolatilityATRMultiplier;
		local_atrvolatility = local_atrfast >= local_atrmultiplied;
		local_indy = local_indy + ", iATR_fast: " + sub_dbl2strbrokerdigits(local_atrfast) + ", iATR_slow: " + sub_dbl2strbrokerdigits(local_atrslow) + ", iATR_mult: " + sub_dbl2strbrokerdigits(local_atrmultiplied);
	}	
	...
	// If we're using iATR as indicator, then check if there's a breakout
	else if (UseIndicatorSwitch == 4 && local_atrvolatility == TRUE)
	{
		// Also use iMA to get short trend
		if (local_isbidgreaterthanima == TRUE)
		{
			local_isbidgreaterthanindy = TRUE;
		}
		// Use dual-direction
		if (ATRDualTrade == TRUE)
			local_pricedirection = 2;
		// Use single-direction
		else 
		{
			// Use Moving Average as short-trend direction
	...
	// Don't check for ATR as we use other ways to find trade signals
		...
		if (OrderMagicNumber() == Magic && OrderCloseTime() == 0) 
		{
			...
			case OP_BUY:
				...
				while (true) 
				{
					...
					if (local_ordertakeprofit < sub_normalizebrokerdigits(local_askpluscommission + stoplevel) && local_askpluscommission + stoplevel - local_ordertakeprofit > TrailingStart) 
					{
						// Set SL and TP
						local_orderstoploss = sub_normalizebrokerdigits(local_bid - stoplevel - AddPriceGap);
						local_ordertakeprofit = sub_normalizebrokerdigits(local_askpluscommission + stoplevel + AddPriceGap);
			...
			case OP_SELL:
				...
				while (true) 
				{
					...
					if (local_ordertakeprofit > sub_normalizebrokerdigits(local_bidminuscommission - stoplevel) && local_ordertakeprofit - local_bidminuscommission + stoplevel > TrailingStart) 
					{					
						// set SL and TP
						local_orderstoploss = sub_normalizebrokerdigits(local_ask + stoplevel + AddPriceGap);
						local_ordertakeprofit = sub_normalizebrokerdigits(local_bidminuscommission - stoplevel - AddPriceGap);
						// Start execution timer
						execution = GetTickCount(); 
						// Send an OrderModify command with adjusted SL and TP
			...
			case OP_BUYSTOP:
				...
				if (local_isbidgreaterthanindy == FALSE) 
				{
					...
					while (true) 
					{
						...
						if (sub_normalizebrokerdigits(local_ask + stoplevel) < OrderOpenPrice() && OrderOpenPrice() - local_ask - stoplevel > TrailingStart) 
						{
							// Start execution timer
							execution = GetTickCount();
			...
			case OP_SELLSTOP:
				...
				if (local_isbidgreaterthanindy == TRUE) 
				{
					...
					while (true) 
					{
						...
						if (sub_normalizebrokerdigits(local_bid - stoplevel) > OrderOpenPrice() && local_bid - stoplevel - OrderOpenPrice() > TrailingStart) 
						{
		...
		{			
			...
			if (ECN_Mode == TRUE) 
			{
				...
				{		
					...
					local_orderstoploss =  local_askplusdistance - StopLoss * Point - AddPriceGap;
					local_ordertakeprofit = local_askplusdistance + TakeProfit * Point + AddPriceGap;
		...
		if (local_pricedirection == 1 || local_pricedirection == 2) 
		{
			...
			if (ECN_Mode) 
			{
				...
				if (OrderSelect(local_orderticket, SELECT_BY_TICKET)) 
				{
					...
					local_orderprice = OrderOpenPrice();
					local_orderstoploss = local_bidminusdistance + StopLoss * Point + AddPriceGap;
					local_ordertakeprofit = local_bidminusdistance - TakeProfit * Point - AddPriceGap;
...
double sub_calculatelotsize()
{
   ...
   double local_availablemoney;
	...
	local_lotsize = MathMin(MathFloor(Risk / 102 * local_availablemoney / StopLoss / lotstep) * lotstep, MaxLots);
	// Empty textstring
	local_textstring = "";
...
double sub_recalculatewrongrisk()
{
	...
	local_maxrisk = MathFloor(local_maxlot * StopLoss / local_availablemoney * 100 / 0.1) * 0.1;
	// Minimum allowed Lot by the broker
	local_minlot = MinLots;
	...
	if (MoneyManagement == TRUE)
	{
		...
		if (Risk < local_minrisk)
		{
			...
			local_textstring = local_textstring + "the broker, StopLoss and Equity. It has now been adjusted accordingly to " + DoubleToStr(local_minrisk,1) + "%";	
			Risk = local_minrisk;
			sub_printandcomment(local_textstring);
		}	
	}
