...
string myCopyRight="作者:老易 QQ:921795";
...
int cnt,i,j; //计数器变量
int TradingDelay=5000; //交易延时变量，1000毫秒
...
void iShowInfo()
{
    ...
    if (myLastError>0)
    {
        iDisplayInfo(InfoDisplay,"LastErrorStr", iGetErrorInfo(myLastError), Corner, 180, 5, 12, "", Red);
    }
    //计算22个基本控单变量
    ...
    if (InfoDisplay==true) 
    {
        iDisplayInfo(InfoDisplay,"Author", myCopyRight, Corner, 18, 15, 8, "", SlateGray);
        iDisplayInfo(InfoDisplay,"TradeInfo", TimeToStr(TimeCurrent(),TIME_DATE|TIME_MINUTES|TIME_SECONDS), 1, 5, 50, 9, "",SlateGray);
        iDisplayInfo(InfoDisplay,"Symbol", Symbol(), Corner, 25, 30, 14, "Arial Bold", DodgerBlue);
        //显示订单信息
        iDisplayInfo(InfoDisplay,Symbol()+"-BuyGroup", "买入组", Corner, 70, 70, 12, "Arial", Red);
        iDisplayInfo(InfoDisplay,Symbol()+"-Ask", DoubleToStr(Ask, Digits), Corner, 70, 90, 12, "Arial", Red);
        iDisplayInfo(InfoDisplay,Symbol()+"-SellGroup", "卖出组", Corner, 5, 70, 12, "Arial", Green);
        iDisplayInfo(InfoDisplay,Symbol()+"-Bid", DoubleToStr(Bid, Digits), Corner, 5, 90, 12, "Arial", Green);
        //显示买入组信息
        iDisplayInfo(InfoDisplay,Symbol()+"-BuyGroup", "买入组", Corner, 70, 70, 12, "Arial", Red);
        iDisplayInfo(InfoDisplay,Symbol()+"-Ask", DoubleToStr(Ask, Digits), Corner, 70, 90, 12, "Arial", Red);
        iDisplayInfo(InfoDisplay,Symbol()+"BuyOrders", BuyGroupOrders+"  "+BuyLimitOrders+"  "+BuyStopOrders, Corner, 80, 110, 10, "Arial", iObjectColor(BuyGroupProfit));
        iDisplayInfo(InfoDisplay,Symbol()+"BuyGroupLots", DoubleToStr(BuyGroupLots, 2), Corner, 80, 125, 10, "Arial", iObjectColor(BuyGroupProfit));
        iDisplayInfo(InfoDisplay,Symbol()+"BuyGroupProfit", DoubleToStr(BuyGroupProfit, 2), Corner, 80, 140, 10, "Arial", iObjectColor(BuyGroupProfit));
        //显示卖出组信息
        iDisplayInfo(InfoDisplay,Symbol()+"-SellGroup", "卖出组", Corner, 5, 70, 12, "Arial", Green);
        iDisplayInfo(InfoDisplay,Symbol()+"-Bid", DoubleToStr(Bid, Digits), Corner, 5, 90, 12, "Arial", Green);
        iDisplayInfo(InfoDisplay,Symbol()+"SellOrders", SellGroupOrders+"  "+SellLimitOrders+"  "+SellStopOrders, Corner, 10, 110, 10, "Arial", iObjectColor(SellGroupProfit));
        iDisplayInfo(InfoDisplay,Symbol()+"SellGroupLots", DoubleToStr(SellGroupLots, 2), Corner, 10, 125, 10, "Arial", iObjectColor(SellGroupProfit));
        iDisplayInfo(InfoDisplay,Symbol()+"SellGroupProfit", DoubleToStr(SellGroupProfit, 2), Corner, 10, 140, 10, "Arial", iObjectColor(SellGroupProfit));
    }
}
...
{
    iShowInfo();
    iDisplayInfo(InfoDisplay,"TradeInfo", "^_^快乐交易^_^      ", Corner, 5, 50, FontSize, "", Olive);
    //初始化预设变量
    Lots=初始开仓量;
...
/*
函    数：组均价
输入参数：myGroupType-组类型，0-买入，1-卖出
输出参数：组均价
算    法：
*/
double iAvgPrice(int myGroupType)
{
    double myReturn=0;
    double myPriceTotal=0;
    double myLotsTotal=0;
    if (myGroupType==OP_BUY && BuyGroupOrders>0)
    {
        for (cnt=0;cnt<OrdersTotal();cnt++)
        {
            if (OrderSelect(cnt,SELECT_BY_POS,MODE_TRADES)
                ...
                && OrderType()==OP_BUY)
            {
                myPriceTotal=myPriceTotal+OrderOpenPrice()*OrderLots();
                myLotsTotal=myLotsTotal+OrderLots();
            }
        }
        if (myLotsTotal!=0) myReturn=myPriceTotal/myLotsTotal;
    }
    if (myGroupType==OP_SELL && SellGroupOrders>0)
    {
        for (cnt=0;cnt<OrdersTotal();cnt++)
        {
            if (OrderSelect(cnt,SELECT_BY_POS,MODE_TRADES)
...
算    法:
*/
int iFileOperation(string myFileName,int myFileType,int myFileMode,string myFileString)
{
    int myHandle; //文件序列号
...
算    法:
*/
int iBatchClose(int myType,int myMagicNum)
{
    iWait(TradingDelay);
    for (cnt=0;cnt<OrdersTotal();cnt++)
    {
...
int iOrdersSort(int myOrderType,int myOrderSort,int myMaxMin,int myMagicNum)
{
    ...
    ArrayInitialize(myOrdersArray, -0.00000001); //初始化订单数组
    //订单数组赋值
...
/*
函    数：在屏幕上显示文字标签
输入参数：bool myInfoDsp 显示开关
          string LableName 标签名称，如果显示多个文本，名称不能相同
...
{
    if (Corner == -1 || myInfoDsp==false) return(0);
