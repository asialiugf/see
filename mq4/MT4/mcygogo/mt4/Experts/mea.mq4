...
void init()
{
	...
	if (dlStopLossPips >0 && dlTakeProfitPips>0 )
	{
	...
	{
		Alert("No Stop loss or TakeProfit!, It's not good Idea");
		return (0);
	}
	...
	//g_LastLots = Gen_Lots();  //Just for OSD info
	...
	string txtTradeLateFri,
	 ...
	 // ----Display Line 1----
	DisplayText("Title1", yLine1 - 5, xCol1, "MEA Demo - Just For EURUSD(H1) ", 13, "Verdana", Red);
...
void OpenBuy(double BuyLots)
{
	...
	//*/
	/* always got error 130 , it's werid
	if (BuyLots != 0 && dBuyStopLossPips > 0 && dBuyTakeProfitPips > 0)
	{
		double dStopLoss = 0, dTakeProfit = 0;
		dStopLoss = NormalizeDouble(Ask - dBuyStopLossPips * PipSize, Digits);
		dTakeProfit = NormalizeDouble(Ask + dBuyTakeProfitPips * PipSize, Digits);
		int order = OrderSend(Symbol(), OP_BUY, BuyLots, Ask, nSlippage, dStopLoss, dStopLoss, CommentTxt, MagicNo, 0, colorOpenBuy);
		if (order > -1 && lFlagUseSound)
		{
			PlaySound(sSoundFileName);
		}
...
void OpenSell(double SellLots)
{
	...
	//*/
	/*
	if (SellLots >= 0 && dSellStopLossPips > 0 && dSellTakeProfitPips > 0)
	{
		double dStopLoss = 0, dTakeProfit = 0;
		dStopLoss = NormalizeDouble(Bid + dSellStopLossPips * PipSize, Digits);
		dTakeProfit = NormalizeDouble(Bid - dSellTakeProfitPips * PipSize, Digits);


		int order = OrderSend(Symbol(), OP_SELL, SellLots, Bid, nSlippage, dStopLoss, dTakeProfit, CommentTxt, MagicNo, 0, colorOpenSell);	
		if (order > -1 && lFlagUseSound)
		{
			PlaySound(sSoundFileName);
		}
