...
//===================================================================================================================================================
extern string EAName             = "OM_2WAY_v3.6c";
//===================================================================================================================================================
extern int MagicNumberBuy        = 123456789;   
extern int MagicNumberSell       = 987654321;   
extern bool   Reverse_Orders     = False;  // If True it will open buy order instead of sell order and vice verca
extern string s1                 = ">>>>>>>>>>>>>>>>>>>>>>>>>>>>";
...
extern double Profit$            = 50;     // Profit in $ to close all open orders
extern double Loss$              = 1000;   // Loss in $ to close all open orders
...
int start()
{
	...
	if (OrdersTotal()>0 && Profit$>0 && NetProfit()>=Profit$)
	{
    	CloseAll();
    	Sleep(1000);
    	if (OrdersTotal()>0) 
 	...
 	if (OrdersTotal()>0 && Loss$>0 && NetProfit()<=-Loss$)
 	{
    	CloseAll();
    	Sleep(1000);
    	if (OrdersTotal()>0)
...
{ 
   ...
   { 
      ...
      { 
            if(OrderType()==OP_BUY)  OrderClose(OrderTicket(),OrderLots(),Bid,2*Spread,Blue); 
            if(OrderType()==OP_SELL) OrderClose(OrderTicket(),OrderLots(),Ask,2*Spread,Red); 
      } 
   }
   return(0); 
}