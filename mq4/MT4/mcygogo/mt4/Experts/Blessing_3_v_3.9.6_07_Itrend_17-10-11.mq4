//+---------------------------------------------------------------------+
//|      Bessing 3 v3.9.6_7 with iTrend indicator by Andreu, 17-10-2011 |
//|                                                    October 11, 2011 |
...
extern string   Version_3_9_6_05    = "EA Settings:";
...
extern int      ITRENDEntry         = 0;        // 0 = Off, 1 = will base entry on ITREND, 2 = will trade in reverse 
...
extern string   LabelITREN          = "ITREND Entry Settings:";
extern string   LabelITRENDTF       = "0:Chart, 1:M1, 2:M5, 3:M15, 4:M30, 5:H1, 6:H4, 7:D1, 8:W1, 9:MN1";
extern int      ITREND_TF           = 0;       // Time frame for ITREND calculation zz
...
{	CS="Waiting for next tick .";      // To display comments while testing, simply use CS = .... and
	...
	if(ITRENDEntry<0||ITRENDEntry>2)ITRENDEntry=0;
...
{	int     CbB          =0;     // Count buy
	...
	{	string sCalcTP="Set New TP:  BE: "+DTS(BEb,Digits);
		...
		{	if(ForceTPPips>0)
			...
			if(BCa<0)
			{	double TPAdj=ND(BCa/PipVal2/nLots,Digits);
				NewTP-=TPAdj;
		...
		{	if(ForceTPPips>0)
			...
			if(BCa<0)
			{	TPAdj=ND(BCa/PipVal2/nLots,Digits);
				NewTP-=TPAdj;
				sCalcTP=sCalcTP+" -BC: ("+DTS(-TPAdj,Digits)+") :";
			}
	...
	{	if(HolShutDown>0&&TimeCurrent()>=HolLast&&HolLast>0)
		...
		{	for(y=0;y<ArraySize(HolArray);y++)
			{	datetime TempHolFirst=StrToTime(Year()+"."+HolArray[y,0]+"."+HolArray[y,1]);
				datetime TempHolLast=StrToTime(Year()+"."+HolArray[y,2]+"."+HolArray[y,3]+" 23:59:59");
				if(TempHolFirst>TempHolLast)
				{	if(TimeCurrent()<TempHolLast)TempHolFirst=StrToTime(DTS(Year()-1,0)+"."+HolArray[y,0]+"."+HolArray[y,1]);
					else TempHolLast=StrToTime(DTS(Year()+1,0)+"."+HolArray[y,2]+"."+HolArray[y,3]+" 23:59:59");
				}
				if(TimeCurrent()<TempHolLast&&(TempHolLast<HolLast||HolLast==0))
				{	HolFirst=TempHolFirst;
					HolLast=TempHolLast;
				}
			}
	...
	//+----------------------------------------------------------------+
	//| Itrend Indicator for Order Entry        zz                     |
	//+----------------------------------------------------------------+
	if(ITRENDEntry>0&&CbT==0&&CpT<2)
	{	double Itrend1=iCustom(NULL,TF[ITREND_TF],"i_Trend",Bands_Mode_0_2,Power_Price_0_6,Price_Type_0_3,Bands_Period,Bands_Deviation, Power_Period, CountBars,0,0);
		double Itrend2=iCustom(NULL,TF[ITREND_TF],"i_Trend",Bands_Mode_0_2,Power_Price_0_6,Price_Type_0_3,Bands_Period,Bands_Deviation, Power_Period, CountBars,1,0);
		if(Itrend1>Itrend2) 		
		{	if(ITRENDEntry==1)
			{	if(ForceMarketCond!=1&&(UseAnyEntry||IndEntry==0||(!UseAnyEntry&&IndEntry>0&&BuyMe)))BuyMe=true;
				else BuyMe=false;
			...
			else if(ITRENDEntry==2)
			{	if(ForceMarketCond!=0&&(UseAnyEntry||IndEntry==0||(!UseAnyEntry&&IndEntry>0&&SellMe)))SellMe=true;
				else SellMe=false;
				if(!UseAnyEntry&&IndEntry>0&&BuyMe&&(!B3Traditional||(B3Traditional&&Trend!=2)))BuyMe=false;
			}
		}
		else if(Itrend1<Itrend2)
		{	if(ITRENDEntry==1)
			{	if(ForceMarketCond!=0&&(UseAnyEntry||IndEntry==0||(!UseAnyEntry&&IndEntry>0&&SellMe)))SellMe=true;
				else SellMe=false;
				if(!UseAnyEntry&&IndEntry>0&&BuyMe&&(!B3Traditional||(B3Traditional&&Trend!=2)))BuyMe=false;
		...
		if(IndEntry>0)IndicatorUsed=IndicatorUsed+UAE;
		IndEntry++;
		IndicatorUsed=IndicatorUsed+" Itrend ";
	}
